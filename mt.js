...
var mtCookieDomain = ".billyclark.net";
var mtCookiePath = "/";
...
function mtCommentOnSubmit(f) {
    if (!mtRequestSubmitted) {
        ...
        if (f.armor)
            f.armor.value = '977064726ed33a0a3e29691323e6603416e168f1';
...
function mtSignIn() {
    ...
    if (is_preview) {
        if ( document['comments_form'] ) {
            ...
            url += '&return_url=http%3A%2F%2Fbillyclark.net%2F';
        }
    } else {
...
function mtSignInOnClick(sign_in_element) {
    ...
    if (el)
        el.innerHTML = 'Signing in... <div class="status-indicator">&nbsp;</div>';
